...
Simulate three replications. Note that the states are cleared and resampled; the rate multipliers are resampled as well. The resulting aligments are stored in files aln\_1.fas, aln\_2.fas, aln\_3.fas.
<<echo=TRUE>>=
for(i in 1:3){
