...
many replicates in parallel by using the {\tt mclapply} method from the {\tt multicore} package (currently not available 
...
Load the multicore package if the operating system is not Windows:
<<echo=TRUE>>=
os.win <- FALSE

if(length(grep("windows", Sys.info()['sysname'],ignore.case=T)) > 0){
    os.win <-TRUE
}
