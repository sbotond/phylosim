...
Simulate three replications. Note that the states are cleared and resampled; the rate multipliers are resampled as well. The resulting alignments are stored in files aln\_1.fas, aln\_2.fas, aln\_3.fas.
<<echo=TRUE>>=
for(i in 1:3){
...
\subsection{Simulating many replications in parallel}

The the speed of the above method for simulating replicates can be improved on a multicore machine by running
many replications in parallel by using the {\tt mclapply} method from the {\tt multicore} package. 
...
The following code illustrates how to simulate many replications in parallel under the JC69+d$\Gamma$ model.
...
Print the resulting \psim objects:
<<echo=TRUE>>=
print(res.objects)
@
